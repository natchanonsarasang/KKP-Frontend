    ...
    const categorizeConversation = async (log: string, status: string): Promise<{ category: string, callback_time?: string }> => {
      ...
      try {
        ...
        const prompt = `
          ...
          6. ลูกค้ายอมจ่าย + บอกวันที่ (Agrees to pay + date)
          7. ลูกค้ายอมจ่าย แต่ไม่บอกวันที่ (Agrees to pay but no date)
        ...
        const response = await fetch('https://api.openai.com/v1/chat/completions', {
          ...
          body: JSON.stringify({
            model: 'gpt-4o-mini',
            messages: [{ role: 'system', content: 'You are an expert debt collection call analyzer.' }, { role: 'user', content: prompt }],
      ...
      if (phoneNumber) {
        ...
        if (debtors && debtors.length > 0) {
          ...
          if (recentItem) {
            ...
            const isRetryable = !pickedUp && [
              'failed', 'no answer', 'no_answer', 'busy', 'error', 'rejected', 'voicemail'

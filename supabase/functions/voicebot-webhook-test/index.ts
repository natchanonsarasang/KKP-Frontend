...
serve(async (req) => {
  ...
  try {
    ...
    // ─── AUTO NEXT-CALL (MAX_CONCURRENT = 1) ──────────────────────────────────
    const MAX_CONCURRENT = 1;
    const activeCount = 0;
    // if (resolvedWorkspaceId) {
    // // 1. นับสายที่กำลังโทรอยู่ใน workspace นี้
    ...
    console.log(`[AutoCall] Active: ${activeCount}, Slots available: ${slots}`);

    if (slots > 0) {
        // 2. ดึง debtors ที่ยังไม่ได้โทร (call_outcome = null หรือ ว่าง)
        const { data: nextDebtors } = await supabase
        .from("debtors")
        .select("id, phone_number, variables")
        .eq("workspace_id", resolvedWorkspaceId)
        .and("call_outcome.is.null,call_outcome.eq.")
        .eq("is_blocked", false)
        .order("created_at", { ascending: true })
        .limit(slots);

        console.log(`[AutoCall] Found ${nextDebtors?.length ?? 0} debtor(s) to call`);

        // 3. ส่งไปให้ voicebot-makecall ทีละคน
        for (const debtor of nextDebtors ?? []) {
        console.log(`[AutoCall] Calling debtor ${debtor.id} → ${debtor.phone_number}`);

        fetch(`${supabaseUrl}/functions/v1/voicebot-makecall`, {
            method: "POST",
            headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${supabaseKey}`,

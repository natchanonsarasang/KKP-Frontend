...
Single-page React app (Vite + React 18 + TypeScript + shadcn-ui + Tailwind). `src/integrations/supabase/client.ts` and `src/integrations/supabase/types.ts` are auto-generated by the Supabase CLI and should not be hand-edited.
...
- Auth is Supabase Auth, used directly via `supabase.auth`.
- Backend logic lives in Deno **Edge Functions** under `supabase/functions/` (voicebot webhooks, call dispatch, Airtable proxy, etc.) with SQL schema in `supabase/migrations/`. These run in Supabase, separate from the Vite frontend. Several of these functions call Lovable's AI Gateway (`LOVABLE_API_KEY`, `ai.gateway.lovable.dev`) for call classification and NL query parsing — this is a functional AI provider dependency, kept intentionally; it is unrelated to the Lovable.dev no-code platform/branding that was removed from the frontend.
